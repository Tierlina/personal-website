...
const mainLinks = [
  { link: './UserButton.tsx', label: 'Intro' },
...
export function DoubleHeader() {
  ...
  const mainItems = mainLinks.map((item, index) => {
    ...
    const activeCls = ' border-dark-lavender rounded-xl';
    function HoverLink({ href, children }: { href: string; children: React.ReactNode }) {
      ...
      return (
        ...
        >
          {children}
          <div
          className={`border-b-3 border-dark-lavender rounded-xl transition-all duration-300 ease-in-out ${hovered ? 'w-full' : 'w-0'} ${isActive ? 'w-full' : ''}`}
          />
        </motion.a>
      );
    }
    ...
    return (
      <HoverLink href={item.link} key={item.label}>
        {item.label}
      </HoverLink>
    );
  });
